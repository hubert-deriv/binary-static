        ...
        $('.currency_wrapper').off('click dblclick').on('click dblclick', function () {
            ...
            if (should_show_confirmation) {
                ...
                if (Currency.isCryptocurrency(currency)) {
                    ...
                    localized_footnote = `${localize('Note:')} ${localize('You are limited to one fiat account. You can change the currency of your fiat account anytime before you make a first-time [_1]', `${clients_country === 'im' ? 'deposit.' : 'deposit or create an MT5 account.'}`)}`;
                }
